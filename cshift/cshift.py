...
class CShift:
    ...
    def _validate_inputs(self):
        assert self.clusters.size == self.groups.size, \
            "clusters and groups must be the same size"
        assert np.isin(self.reference, self.groups).all(), \
            "reference must be a subset of groups"
    ...
    def fit(self):
        ...
        self.pval_matrix = np.stack([
            aggregate_chisquare_test(
                ...
                self.distributions[i]) for i in np.arange(self.g_size)
        ])
        self.pcc_matrix = np.stack([
            percent_change(
                self.distributions[self.ref_idx].mean(axis=0),
                self.distributions[i]) for i in np.arange(self.g_size)
        ])
